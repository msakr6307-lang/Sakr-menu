...
// --- تعريف المتغيرات (تم التأكد من استخدامها جميعاً) ---
...
+ (void)tSwitch:(UIButton *)s {
    vC.hidden = YES; vV.hidden = YES; vJ.hidden = YES; vR.hidden = YES;
...
+ (void)fToggle:(UIButton *)b {
    // استخدام المتغيرات لمنع خطأ unused variable
...
static void initMustafaEngine() {
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(10 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        [MustafaFinalClean addB:@"SMART HEADSHOT 🎯" y:0 tag:1 p:vC];
        [MustafaFinalClean addB:@"SMART BODYSHOT 👕" y:50 tag:2 p:vC];
        [MustafaFinalClean addB:@"WALLBANG 🧱" y:100 tag:3 p:vC];
        [MustafaFinalClean addB:@"INFINITE AMMO 🔫" y:150 tag:4 p:vC];

        [MustafaFinalClean addB:@"ESP BOX" y:0 tag:101 p:vV];
        [MustafaFinalClean addB:@"ESP SKELETON" y:45 tag:102 p:vV];
        [MustafaFinalClean addB:@"ESP LINES" y:90 tag:103 p:vV];

        dLab = [[UILabel alloc] initWithFrame:CGRectMake(10, 140, 360, 30)];
        dLab.text = @"ESP Distance: 150M"; dLab.textColor = [UIColor whiteColor]; [vV addSubview:dLab];

        UISlider *sd = [[UISlider alloc] initWithFrame:CGRectMake(10, 170, 360, 40)];
        sd.minimumValue = 50; sd.maximumValue = 250; sd.value = 150;
        sd.minimumTrackTintColor = [UIColor cyanColor];
        [sd addTarget:[MustafaFinalClean class] action:@selector(sSlide:) forControlEvents:UIControlEventValueChanged];
        [vV addSubview:sd];
