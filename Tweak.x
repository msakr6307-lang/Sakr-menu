...
void hook_on_gui(void *self) {
    orig_on_gui(self);
    // Draw ESP elements here
}
...
void installHooks() {
    ...
    MSHookFunction((void *)(base + OFFSET_ON_GUI), (void *)&hook_on_gui, (void **)&orig_on_gui);
...
- (void)moveFalcon:(UIPanGestureRecognizer *)sender {
    ...
    CGFloat maxX = self.bounds.size.width - sender.view.frame.size.width;
    CGFloat maxY = self.bounds.size.height - sender.view.frame.size.height;
    newCenter.x = MAX(30, MIN(maxX, newCenter.x));
    newCenter.y = MAX(30, MIN(maxY, newCenter.y));
