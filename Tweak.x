...
static const char* (*orig_get_name)(void *player);
static void* (*orig_get_position)(void *player);
static void (*orig_initialize_players)(void *self, void *near, void *search);
static float (*orig_get_distance)(void *self);
static void (*orig_on_gui)(void *self);
...
void hook_initialize_players(void *self, void *near, void *search) {
    ...
    if (isEspBoxOn || isEspNameOn || isEspHealthOn || isEspDistanceOn) {
        NSLog(@"[ESP] Players list updated.");
    }
}
...
void installHooks() {
    ...
    void *closestPlayer = (void *)(base + OFFSET_GET_CLOSEST_PLAYER);
    if (closestPlayer) MSHookFunction(closestPlayer, (void *)&hook_find_target, (void **)&orig_get_closest_player);
    void *setTarget = (void *)(base + OFFSET_SET_TARGET);
    if (setTarget) MSHookFunction(setTarget, (void *)&hook_find_target, (void **)&orig_set_target);
    void *lockTarget = (void *)(base + OFFSET_LOCK_TARGET);
    if (lockTarget) MSHookFunction(lockTarget, (void *)&hook_find_target, (void **)&orig_lock_target);
...
- (void)setupMenu {
    ...
    NSArray *tabs = @[@"⚔️ WEAPON", @"⛏️ MINING", @"🛡️ PLAYER", @"🎯 AIMBOT", @"👁️ ESP"];
    for (int i = 0; i < tabs.count; i++) {
        UIButton *tabBtn = [UIButton buttonWithType:UIButtonTypeSystem];
        tabBtn.frame = CGRectMake(10 + (i * 56), 50, 54, 35);
...
- (void)onTabClick:(UIButton *)sender {
    NSArray *tabs = @[@"WEAPON", @"MINING", @"PLAYER", @"AIMBOT", @"ESP"];
...
- (void)showTab:(NSString *)tab {
    ...
    if ([tab isEqualToString:@"WEAPON"]) {
        ...
        aimbotBtn = [self createButton:@"🎯 Aimb [self createButton:@"🏎️ Speed Hack" y:y action:@selector(toggleSpeedHack)];
    }
    else if ([tab isEqualToString:@"AIMBOT"]) {
        aimbotBtn = [self createButton:@"🎯 Aimbot" y:y action:@selector(toggleAimbot)];
    }
    else if ([tab isEqualToString:@"ESP"]) {
        espBoxBtn = [self createButton:@"📦 Player Box" y:y action:@selector(toggleEspot" y:y action:@selector(toggleAimbot)];
    }
    else if ([tab isEqualToString:@"ESP"]) {
        ...
        espHealthBtn = [self createButton:@"❤️ Player Health" y:y action:@selector(toggleEspHealth)];Box)]; y += 50;
        espNameBtn = [self createButton:@"🏷️ Player Name" y:y action:@selector(toggleEspName)]; y += 50;
...
- (UIButton *)createButton:(NSString *)title y:(CGFloat)y action:(SEL)action {
    ...
    [btn setTitle:[NSString stringWithFormat:@"%@ 🔴 OFFButton *btn = [UIButton buttonWithType:UIButtonTypeSystem];
    btn.frame = CGRectMake(15, y, 250, 42);
    btn.backgroundColor = [UIColor colorWithWhite:0.2 alpha:1];
    btn.layer.cornerRadius = 8;
    ...
    [btn addTarget:self action:action forControlEvents:UIC", title] forState:UIControlStateNormal];
    [btn setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
    btn.titleLabel.font = [UIFont boldSystemFontOfSize:14];
...
- (void)updateButton:(UIButton *)btn isOn:(BOOL)isOn title:(NSString *)title {
    dispatch_async(dispatch_get_main_queue(), ^{
        NSString *newTitle = [NSString stringWithFormat:@"%@ %@",ontrolEventTouchUpInside];
    [contentView addSubview:btn];
    return btn;
}
...
- (void)toggleOneHitKill { isOneHitKillOn = !isOneHitKillOn ========== Toggles ==========
- (void)toggleMagicBullet { isMagicBulletOn = !isMagicBulletOn; [self updateButton:magicBulletBtn isOn:isMagicBulletOn title:@"✨ Magic Bullet"]; }
- (void)toggleOneHitKill { isOneHitKillOn = !isOneHitKillOn; [self updateButton:oneHitKillBtn isOn:isOneHitKillOn title:@"💀 1 Hit Kill"]; }
- (void)toggleInfiniteAmmo { isInfiniteAmmoOn = !isInfiniteAmmoOn; [self updateButton:infiniteAmmoBtn isOn:isIn; [self updateButton:oneHitKillBtn isOn:isOneHitKillOn title:@"💀 1 Hit Kill"]; }
...
- (void)toggleAutoMine {finiteAmmoOn title:@"🔫 Infinite Ammo"]; }
- (void)toggleRapidFire { isRapidFireOn = !isRapidFireOn; [self updateButton:rapidFireBtn isOn:isRapidFireOn title:@"⚡ Rapid Fire"]; }
- (void)toggleAutoMine { isAutoMineOn = !isAutoMineOn; [self updateButton:autoMineBtn isOn:isAutoMineOn title:@"🤖 Auto Mine"]; }
- (void)toggleGodMode { is isAutoMineOn = !isAutoMineOn; [self updateButton:autoMineBtn isOn:isAutoMineOn title:@"🤖 Auto Mine"]; }
- (void)toggleGodMode { isGodModeOn = !isGodModeOn; [self updateButton:godModeBtn isOn:isGodModeOn title:@"🛡️ God Mode"]; }
- (void)toggleEnergy { isEnergyOn = !isEnergyOn; [self updateButton:energyBtn isGodModeOn = !isGodModeOn; [self updateButton:godModeBtn isOn:isGodModeOn title:@"🛡️ God Mode"]; }
- (void)toggleEnergy { isEnergyOn = !isEnergyOn; [self updateButton:energyBtn isOn:isEnergyOn title:@"⚡ Energy"]; }
- (void)toggleSpeedHack { isSpeedHackOn = !isSpeedHackOn; [self updateButton:speedOn:isEnergyOn title:@"⚡ Energy"]; }
- (void)toggleSpeedHack { isSpeedHackOn = !isSpeedHackOn; [self updateButton:speedHackBtn isOn:isSpeedHackOn title:@"🏎️ Speed Hack"]; }
- (void)toggleAimbot { isAimbotOn = !isAimbotOn; [self updateButton:aimbotBtn isOn:isAimbotOnHackBtn isOn:isSpeedHackOn title:@"🏎️ Speed Hack"]; }
- (void)toggleAimbot { isAimbotOn = !isAimbotOn; [self updateButton:aimbotBtn isOn:isAimbotOn title:@"🎯 Aimbot"]; }
- (void)toggleEspBox { isEspBoxOn = !isEspBoxOn; [self updateButton:espBoxBtn isOn:isEsp title:@"🎯 Aimbot"]; }
...
- (voidBoxOn title:@"📦 Player Box"]; }
- (void)toggleEspName { isEspNameOn = !isEspNameOn; [self updateButton:espNameBtn isOn:isEspNameOn title:@"🏷️ Player Name"]; }
- (void)toggleEspHealth { isEspHealthOn = !isEspHealthOn;)toggleEspHealth { isEspHealthOn = !isEspHealthOn; [self updateButton:espHealthBtn isOn:isEspHealthOn title:@"❤️ Player Health"]; }
...
__attribute__((constructor))
 [self updateButton:espHealthBtn isOn:isEspHealthOn title:@"❤️ Player Health"]; }
- (void)toggleEspDistance { isEspDistanceOn = !isEspDistanceOn; [self updateButton:espDistanceBtn isOn:isEspDistanceOn title:@"📏 Distance"]; }
